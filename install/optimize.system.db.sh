#!/bin/sh
# optimize system
# 不可与 optimize.system.nginx.sh 同时执行
...
# 磁盘调度策略
echo -e "3. 磁盘调度策略（${C_BRed}请检查并手动修改${C_None}）"
echo "echo deadline > /sys/block/<dev name>/queue/scheduler"
echo "高版本内核(>4)采用了blk-mq调度，不需要修改"

# 磁盘挂载需要使用 noatime
echo -e "4. 磁盘挂载需要使用 noatime（${C_BRed}请检查并手动修改${C_None}）"
echo "修改 /etc/fstab 在数据库所在盘的 defaults后增加 “,noatime”"
echo "如：/dev/vdb /data xfs defaults,noatime 0 0"
echo "# mount -o remount /data"
